...
export default function CarouselGenerator() {
  ...
  const duplicateSlide = useCallback(() => {
    const clone: SlideData = { ...activeSlide, id: crypto.randomUUID() };
  ...
  const moveSlide = useCallback(
    (fromIndex: number, dir: -1 | 1) => {
      ...
      const newSlides = [...slides];
      [newSlides[fromIndex], newSlides[newIndex]] = [newSlides[newIndex], newSlides[fromIndex]];
      setSlides(newSlides);
      if (fromIndex === activeSlideIndex) setActiveSlideIndex(newIndex);
      else if (newIndex === activeSlideIndex) setActiveSlideIndex(fromIndex);
    },
    [activeSlideIndex, slides]
  );
  ...
  const reorderSlide = useCallback((fromIndex: number, toIndex: number) => {
    ...
    setSlides(newSlides);
    if (activeSlideIndex === fromIndex) setActiveSlideIndex(toIndex);
    else if (fromIndex < activeSlideIndex && toIndex >= activeSlideIndex) setActiveSlideIndex(activeSlideIndex - 1);
    else if (fromIndex > activeSlideIndex && toIndex <= activeSlideIndex) setActiveSlideIndex(activeSlideIndex + 1);
  }, [activeSlideIndex, slides]);
  ...
  return (
    <>
      {/* Action bar */}
      <div className="flex items-center justify-end gap-2 px-5 py-2 border-b border-white/[0.06] bg-[#0c0c0f]">
        <button
          onClick={() => {
          ...
          <div className="flex-1 overflow-y-auto sidebar-scroll p-3">
            ...
            {sidebarTab === "theme" && (
              <div className="space-y-4">
                <div className="section-card">
                  <h3 className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider mb-3">Theme</h3>
                  <ThemePicker selected={settings.theme} onSelect={handleThemeChange} />
                </div>

                <div className="section-card">
                  <h3 className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider mb-3">Aspect Ratio</h3>
                ...
                <div className="section-card">
                  <h3 className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider mb-3">Text Alignment</h3>
                  <div className="flex gap-2">
            ...
            {sidebarTab === "settings" && (
              <div className="section-card space-y-4">
                <h3 className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Branding</h3>
                <div>
                  <label className="block text-xs font-medium text-zinc-500 mb-1.5">Author Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-zinc-500 mb-1.5">Handle</label>
                  <input
                    type="text"
                    value={settings.authorHandle}
                    onChange={(e) => setSettings((s) => ({ ...s, authorHandle: e.target.value }))}
                    className="w-full bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500/50 input-glow transition-all placeholder:text-zinc-600"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-zinc-500 mb-1.5">Brand Label</label>
                  <input
                    type="text"
                    value={settings.brandLabel}
                    onChange={(e) => setSettings((s) => ({ ...s, brandLabel: e.target.value }))}
                    className="w-full bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500/50 input-glow transition-all placeholder:text-zinc-600"
                  />
                </div>
              </div>
                ...
                >
                  <button
                    ...
                    className={`relative rounded-lg overflow-hidden transition-all cursor-grab active:cursor-grabbing ${
                      i === activeSlideIndex ? "thumb-active" : "ring-1 ring-white/[0.08] hover:ring-white/[0.2]"
